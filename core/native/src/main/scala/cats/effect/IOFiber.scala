...
private final class IOFiber[A](
  ...
  private[this] var stack: Array[Object] = _
    ...
    if (resume()) {
      ...
      if (isUnmasked()) {
        ...
        IO.async_[Unit] { fin =>
          ...
          stack = NativeStack.pushObject(stack, fin)
      ...
      (cur0.tag: @switch) match {
        ...
        case 6 =>
          ...
          (ioe.tag: @switch) match {
            ...
            case _ =>
              stack = NativeStack.pushObjectCont(stack, f, MapK)
        ...
        case 7 =>
          ...
          (ioe.tag: @switch) match {
            ...
            case _ =>
              stack = NativeStack.pushObjectCont(stack, f, FlatMapK)
              runLoop(ioe, nextCancelation, nextAutoCede)
          }
        ...
        case 8 =>
          ...
          (ioa.tag: @switch) match {
            ...
            case _ =>
              stack = NativeStack.pushCont(stack, AttemptK)
        ...
        case 9 =>
          ...
          stack = NativeStack.pushObjectCont(stack, cur.f, HandleErrorWithK)
        ...
        case 11 =>
           ...
           */
          stack = NativeStack.pushCont(stack, OnCancelK)
          runLoop(cur.ioa, nextCancelation, nextAutoCede)
        ...
        case 12 =>
           ...
           */
          stack = NativeStack.pushCont(stack, UncancelableK)
          runLoop(next, nextCancelation, nextAutoCede)
        ...
        case 13 =>
          ...
          if (masks == cur.id && (self eq cur.self)) {
             ...
             */
            stack = NativeStack.pushCont(stack, UnmaskK)
          }
                  ...
                  if (!shouldFinalize()) {
                    ...
                    result match {
                      case Left(t) =>
                        ...
                        stack = NativeStack.pushObject(stack, t)
                      case Right(a) =>
                        resumeTag = AsyncContinueSuccessfulR
                        stack = NativeStack.pushObject(stack, a.asInstanceOf[AnyRef])
                    }
                  } else {
                    /*
  ...
  private[this] def prepareFiberForCancelation(cb: Either[Throwable, Unit] => Unit): IO[Any] = {
    if (!finalizers.isEmpty()) {
      if (!finalizing) {
        ...
        stack = NativeStack.create(32)
        stack = NativeStack.pushObjectCont(stack, cb, CancelationLoopK)
  ...
  private[this] def succeeded(result: Any, depth: Int): IO[Any] =
    (NativeStack.popCont(stack): @switch) match {
      case 0 => // mapK
        val f = NativeStack.popObject(stack).asInstanceOf[Any => Any]
      ...
      case 1 => // flatMapK
        val f = NativeStack.popObject(stack).asInstanceOf[Any => IO[Any]]
  ...
  private[this] def failed(error: Throwable, depth: Int): IO[Any] = {
    ...
    (NativeStack.popCont(stack): @switch) match {
      ...
      case 5 => // handleErrorWithK
        val f = NativeStack.popObject(stack).asInstanceOf[Throwable => IO[Any]]
  ...
  private[this] def execR(): Unit = {
    ...
    if (canceled) {
      ...
      stack = NativeStack.create(32)
      stack = NativeStack.pushCont(stack, RunTerminusK)
  ...
  private[this] def asyncContinueSuccessfulR(): Unit = {
    val a = NativeStack.popObject(stack).asInstanceOf[Any]
  ...
  private[this] def asyncContinueFailedR(): Unit = {
    val t = NativeStack.popObject(stack).asInstanceOf[Throwable]
  ...
  private[this] def asyncContinueCanceledWithFinalizerR(): Unit = {
    val cb = NativeStack.popObject(stack).asInstanceOf[Either[Throwable, Unit] => Unit]
  ...
  private[this] def blockingR(): Unit = {
    ...
    if (isStackTracing) {
      ...
      NativeStack.popObject(stack).asInstanceOf[WeakBag.Handle].deregister()
    }

    if (error == null) {
      resumeTag = AsyncContinueSuccessfulR
      stack = NativeStack.pushObject(stack, r.asInstanceOf[AnyRef])
    } else {
      resumeTag = AsyncContinueFailedR
      stack = NativeStack.pushObject(stack, error)
    }
    val ec = currentCtx
    scheduleOnForeignEC(ec, this)
  }
  ...
  private[this] def cancelationLoopSuccessK(): IO[Any] = {
    if (!finalizers.isEmpty()) {
      ...
      stack = NativeStack.pushCont(stack, CancelationLoopK)
      finalizers.pop()
    } else {
      // The last finalizer is done executing.

      // Unblock the canceler of this fiber.
      val cb = NativeStack.popObject(stack)
      if (cb != null) {
  ...
  private[this] def evalOnSuccessK(result: Any): IO[Any] = {
    ...
    val ec = NativeStack.popObject(stack).asInstanceOf[ExecutionContext]
    currentCtx = ec

    if (!shouldFinalize()) {
      resumeTag = AsyncContinueSuccessfulR
      stack = NativeStack.pushObject(stack, result.asInstanceOf[AnyRef])
      scheduleOnForeignEC(ec, this)
      IO.EndFiber
    } else {
